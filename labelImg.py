...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def loadLabels(self, shapes):
        #[('left_eye', [(25, 29), (25, 29), (25, 29), (25, 29)], None, None, False), ('left_eye', [(34, 55), (34, 55), (34, 55), (34, 55)], None, None, False)]
        s = []
        for label, points, line_color, fill_color, difficult in shapes:
            ...
            shape.fill_color = QColor(255,0,0,255)
    ...
    def saveLabels(self, annotationFilePath):
        ...
        try:
            if ustr(annotationFilePath[-4:]) != ".pts":
                annotationFilePath += '.pts'
            print ('Img: ' + self.filePath + ' -> Its pts: ' + annotationFilePath)
            pts_info = []
            for item_obj in shapes:
                pts_coor = item_obj['points'][0]
                pts_x = pts_coor[0]
                pts_y = pts_coor[1]
                pts_info.append('%.2f'%pts_x + ' ' + '%.2f'%pts_y)
            open(annotationFilePath, "w").write("\n".join(pts_info))
    ...
    def newShape(self):
        ...
        """
        item = self.labelList.count()
        cur_pts_count = self.labelList.count()
        if cur_pts_count == 0:
            text = '0'
        else:
            text = str(cur_pts_count)
    ...
    def loadFile(self, filePath=None):
        ...
        if unicodeFilePath and os.path.exists(unicodeFilePath):
            ...
            if self.defaultSaveDir is not None:
                ...
                ptsPath = os.path.join(self.defaultSaveDir, basename + '.pts')
                self.loadPtsByFilename(ptsPath)
            else:
                ptsPath = os.path.splitext(filePath)[0] + '.pts'
                self.loadPtsByFilename(ptsPath)
        ...
        if self.usingPascalVocFormat:
            ...
            if filename:
                if isinstance(filename, (tuple, list)):
                    filename = filename[0]
            self.loadPtsByFilename(filename)
    ...
    def loadPredefinedClasses(self, predefClassesFile):
        ...
        if os.path.exists(predefClassesFile):
            info_line = []
            with open(predefClassesFile, 'r') as f:
                info_line = f.readlines()
            pts_num = int(info_line[0].strip())
        self.labelHist = [str(i) for i in range(pts_num)]

    def loadPtsByFilename(self, xmlPath):
        if self.filePath is None:
            return
        ...
        for idx, line in enumerate(info_lines):
            ...
            pts_x = float(line.strip().split(' ')[0])
            pts_y = float(line.strip().split(' ')[1])

            pts_item = (str(idx), [(pts_x, pts_y)], None, None, False)
            shapes.append(pts_item)

        #[('left_eye', [(25, 29), (25, 29), (25, 29), (25, 29)], None, None, False), ('left_eye', [(34, 55), (34, 55), (34, 55), (34, 55)], None, None, False)]
        #tVocParseReader = PascalVocReader(xmlPath)
        #shapes = tVocParseReader.getShapes()
        self.loadLabels(shapes)
        self.canvas.verified = True #tVocParseReader.verified
